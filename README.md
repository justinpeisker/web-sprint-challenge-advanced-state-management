...
1. What problem does the context API help solve?
  ...
  Context API helps solve the problem of managing state with large applications. It is especially useful when many components are sharing data. It is also very useful when you would need to prop drill through multiple components to give your desired component access to data. In this case context API might be more appropriate, opposed to prop drilling.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

  Actions- Actions are objects that have 2 parameters, a type and a payload.  They contain information that gets passed to the reducer
...
3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
  ...
  Redux-thunk is a type of middleware.  It allows us to create complex actions that return functions (i.e. API calls).

4. What is your favorite state management system you've learned and this sprint? Please explain why!

  So far I see pros and cons with each type of state management system.  But if I have to choose between prop drilling, redux and context API, I think I would choose redux.  To me it makes more sense.  Simply go to the component you're working with, connect redux and pass in the state that's needed for that particular component.  That is why I would choose redux over the other state management systems.
